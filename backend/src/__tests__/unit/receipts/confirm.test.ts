...
describe('confirmReceipt', () => {
  ...
  it('allows re-confirm when ai_stage is messaging before messages are sent', async () => {
    mockSupabase.__pushMockResultForTable('events', {
      data: { ...PARSED_EVENT_ROW, ai_stage: 'messaging' },
      error: null,
    });
    mockSupabase.__pushMockResultForTable('events', {
      data: [{ id: EVENT_ID }],
    ...
    const result = await confirmReceipt(USER_ID, {
      event_id: EVENT_ID,
      items: [{ name: 'Burger', price: 12, quantity: 1 }],
      additional_charges: [],
      discounts: [],
      tax: 0,
      fees: 0,
  ...
  it('allows re-confirm after expenses shared when ai_stage is complete', async () => {
    mockSupabase.__pushMockResultForTable('events', {
      data: {
        ...PARSED_EVENT_ROW,
        status: 'sent',
        ai_stage: 'complete',
        messages_sent_at: '2026-01-03T00:00:00.000Z',
      },
      error: null,
    });
