...
doppler run -- npm run smoke:settlement       # self-report, confirm, dispute, nudge, mark-paid (E09-S01)
doppler run -- npm run smoke:notifications    # inbox list, unread count, mark-read (E10-S02)
...
6. Run `verify-deployment-schema.sql` on staging; run smoke tests (`smoke:receipts-confirm`, `smoke:splits`, `smoke:expenses-reset`, `smoke:messages-preview`, `smoke:split-revision`, `smoke:settlement`, `smoke:notifications`).
