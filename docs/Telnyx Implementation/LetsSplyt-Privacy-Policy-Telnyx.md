...
> **Version note:** This version applies when LetsSplyt uses **Telnyx + custom OTP** for SMS delivery. It differs from the Twilio version in three places: Section 5.2 (service providers table), Section 6 (data retention — adds OTP codes row), and Section 12 (data location providers list). All other content is identical.
...
|---|---|---|---|
| **Telnyx** | Delivering OTP verification codes and payment request SMS to your phone | Phone number (encrypted in transit) | [telnyx.com/privacy-policy](https://telnyx.com/privacy-policy) |
| **Supabase** | Database and file storage (hosted in the United States) | All account and event data, including temporary OTP codes (deleted after 10 minutes) | [supabase.com/privacy](https://supabase.com/privacy) |
...
**Note on OTP verification:** LetsSplyt generates and verifies one-time passcodes using its own backend systems. The OTP code is stored temporarily in our database (Supabase) and expires automatically after 10 minutes. The SMS delivering the code is sent via Telnyx. Neither Telnyx nor any other third party is involved in code generation or verification.
...
| **OTP verification codes** | **10 minutes, then automatically and permanently deleted** | **Temporary code used solely to verify phone number ownership; no purpose after verification or expiry** |
...
- **OTP codes** are stored with a strict 10-minute expiry enforced at the database level and are deleted immediately upon successful verification or expiry, whichever comes first.
...
Your data is stored and processed in the United States. Our service providers (Telnyx, Supabase, Google) operate globally and may process certain data outside the United States. Where they do so, we rely on their data processing terms to ensure equivalent standards of data protection are maintained.
