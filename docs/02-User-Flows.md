...
| P09 | Add member manually — with phone number | From Event Detail (joining view) — **+ Add manually** → `AddMembersSheet` → **Contacts** tab (multi-select) or **By name** tab (name + optional phone). Payer vouches — no OTP. Server hashes phone and looks up `users.phone_hash`. **Registered:** participant linked via `user_id`. **Unregistered:** `guest_pii` stores phone for SMS. SMS invite sent when messages are sent (WhatsApp-first, SMS fallback). | Fast path for people in contacts; manual rows cover anyone else. **MVP** |
| P10 | Add member manually — name only (no phone) | Same sheet — **By name** tab, leave phone blank. No message is ever sent. **Manual** badge in member list. Payer marks cash payment on their behalf after split. | Cash payers and in-person guests. **MVP** |
| P11 | Remove participant | Before event lock — from Event Detail (joining view), remove someone who joined by mistake. | Wrong person scanned, duplicate entry. Payer needs control before split begins. **MVP** |
...
| P14 | Lock event | Tap **Lock event →** when all intended members are in. This: (1) expires the join token immediately, (2) prevents any new participants from joining, (3) prevents existing participants from leaving. **Scan receipt** and **Enter total** are disabled until the event is locked. After lock, Event Detail footer shows those CTAs side by side (creator only). | Splitting before membership is finalised creates undefined shares. Locking is the mandatory gate before any financial calculation. **Critical Path** |
...
| P16 | Review & correct parsed items | Edit misread item names, prices, quantities. Add missing items. Lines A1 could not read reliably appear as **Unreadable line** (low confidence) for payer correction. Shown only in itemised mode. | AI is not perfect. Bad lighting, crumpled receipt, unusual format = parsing errors. Payer review before splitting prevents errors propagating to everyone's amounts. **Critical Path** |
...
| P20a | Edit split after messages already sent | Same **Edit share** footer as pre-send. **Receipt events:** Item Review → Fair play (split entry) → Review split → **Save and notify**. **Manual-total events:** Fair play directly. Only participants whose amounts changed get revision SMS. Edit blocked when any payment is self-reported, confirmed, or settled; dispute (back to pending) re-opens edit if nothing else blocks. **MVP** |
...
| P34 | Delete account & data | Settings tab → **Delete account** → warn screen (balance gate: blocked when Dashboard `you_owe > 0`) → confirm screen (type DELETE) → `POST /users/me/delete`. Tombstones profile, deletes handles, anonymises participant names, removes auth user. | GDPR erasure + settlement integrity. **Legally Required** |
...
**Payer side:** P01 → P04 → P07 → P08 (QR + Event Detail joining view) → [optional: P09/P10 manual add] → P14 (lock event) → P15 → **Scanned:** P15a → **Itemised:** P16 → P18 → P16b → P20 | **Custom/Even:** P16b → P20 || **Manual total:** P17 → P16b (Even pre-selected) → P20 → P21 → P21a → P22 → [Home P30/P31 or Events P28] → P24 (Event Detail settlement) → P25 (confirm payment)
...
**Note on P14:** The **Lock event** action is the hard gate between member formation and split calculation. No split can begin until the payer explicitly locks. This is enforced in the UI (Scan receipt button disabled) and in the API (split endpoints check event lock status).
