...
**Display names:** Registered members (`participants.user_id` set) show their **live** `users.display_name` in event member lists — `getEventById` resolves from `users` on read. `PATCH /users/me` also writes the new name to all linked `participants` rows so SMS, breakdown pages, and Realtime subscriptions stay consistent. Pure guests keep the payer- or browser-supplied `participants.display_name` snapshot.
...
| File Storage | Supabase Storage (S3-compatible) | Included in the Supabase plan. Stores receipt images (uploaded at scan time, referenced by S3 key in `receipt_items`). Split breakdown for SMS recipients is server-rendered HTML at `/split/:token` — not uploaded to Storage on send. Legacy canvas split-image code exists but is not on the delivery path. | letssplyt-dev Supabase project storage | letssplyt-staging project storage | letssplyt-production project storage |
| Auth | Supabase Auth (JWT) + custom OTP | Phone-native authentication with no email friction. LetsSplyt generates OTP and sends it via `SMS_PROVIDER` (Telnyx in staging). Supabase Auth issues JWTs. The mobile app stores the refresh token in Expo SecureStore — never in AsyncStorage or localStorage. | Dev bypass or on-net Telnyx | Live Telnyx OTP to real US mobiles (confirmed 2026-09-26) | Same Telnyx sender as staging |
...
| Messaging | Telnyx SMS via `createSMSProvider()` (Twilio kept as rollback) | Custom OTP + payment-request SMS. Factory selects `telnyx` or `twilio` from `SMS_PROVIDER`. Staging already delivers off-net to US mobiles (2026-09-26). Production reuses that sender; 10DLC only if the number is a US local long-code. | On-net Telnyx or Twilio magic numbers | Live Telnyx to real US phones | Same Telnyx sender; webhook → production `APP_URL` |
...
| SMS (Telnyx) | Dev on-net numbers or `OTP_DEV_BYPASS`. Twilio magic numbers only if `SMS_PROVIDER=twilio`. | Live Telnyx — OTP and payment SMS delivering to real US mobiles (confirmed 2026-09-26). | Same Telnyx sender as staging. 10DLC only if that number is a local long-code. Point Messaging Profile webhook at production `APP_URL`. |
