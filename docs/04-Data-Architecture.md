...
### 3.10b `user_notifications`

In-app notification center (distinct from `notification_log` SMS/push delivery tracking). One row per inbox item shown in the mobile bell + `NotificationsScreen`.
...
**Visibility (enforced in `inbox-notification.service.ts`):**
