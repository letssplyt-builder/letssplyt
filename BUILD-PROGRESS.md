...
**Staging (off-net) — live (confirmed 2026-09-26)**
- [x] `SMS_PROVIDER=telnyx` in Doppler staging
- [x] OTP + payment SMS delivering to real US mobiles (all numbers tested so far)
- [ ] Confirm sender registration in Telnyx Mission Control (toll-free **Verified**, or 10DLC campaign **ACTIVE** if this is a local long-code). Off-net delivery already works; this is paperwork, not a send-path gap.
- [ ] Webhook: `{APP_URL}/api/v1/webhooks/telnyx/messaging` on the sender’s Messaging Profile
- [ ] Real STOP/START from handset
- [ ] Run `npm run smoke:messages-preview` with live credentials

**Production — reuse the staging sender (decision 2026-09-26)**
- [ ] Set production `TELNYX_FROM_NUMBER` to the **same** staging Telnyx number. Do not buy a second number unless we later split environments.
- [ ] At cutover, point that number’s Messaging Profile webhook at production `{APP_URL}/api/v1/webhooks/telnyx/messaging`. Do not send from staging and production on this number at the same time.
- [ ] 10DLC brand + campaign — **only if** the shared number is a US local long-code. Skip if it is an already-delivering toll-free (TFN).
- [x] Legal: published Privacy Policy is the Telnyx + custom-OTP version; `cd mobile && npm run sync:legal-docs` run 2026-09-26
...
- [2026-06-09] — E05-S04 refinement — `AddMembersSheet` tabbed bottom sheet (**Contacts** | **By name**): multi-select contacts, single initial manual row, batch **Done**, invalid-phone validation, partial-failure toast. Removed clipboard paste and inline `GroupBuilderPanel`. User-facing **event**/**members** wording. Docs: 01, 02, 08, 12. Tests: AddMembersSheet (16 cases), groupBuilder.utils, EventDetailScreen batch add.
...
- [2026-06-07] — E12-S02 — Sentry (@sentry/node + react-native), structured Pino logging with PII scrub, debug sentry-test route, setup guide §2.4B; dev DSN verified in Sentry UI. Backend 400 tests.
- [2026-06-07] — Post-d133f19 fixes — Name-only / no-phone members: `messageFlow.ts` skips Message Preview and completes event without SMS (`skipped_no_phone`); MessagePreview auto-complete when previews empty; Event Detail send uses same routing. SplitReview: read-only Member | Owes ledger, event title subtitle, **Complete event →** CTA when no SMS recipients. Web join: full `privacy.html` / `terms.html` generated by `npm run sync:legal-docs` from `docs/LetsSplyt-*.md`. Tests: `messageFlow`, SplitReview, MessagePreview, `legal-pages`; docs 08, 11. Backend 412 + mobile 272 tests.
- [2026-06-07] — Mobile TypeScript strictness + optional Sentry wrap: `withOptionalSentryWrap` skips `Sentry.wrap` when `EXPO_PUBLIC_SENTRY_DSN` unset; `replaceInEventFlow` / typed nested-tab navigation; API body casts; `canParticipantPayShare` treats `undefined` owed amount as blocked. Tests: `sentryAppRoot`, eventFlowNavigation, eventSplitFooter. Docs: 11 §Sentry. Mobile 275 tests.
- [2026-09-26] — Ops/docs — Staging Telnyx is live: `SMS_PROVIDER=telnyx`, OTP and payment SMS delivering to all US numbers tested. Production will **reuse the same Telnyx sender** (no second number, no mandatory 10DLC unless that number is a local long-code). Remaining SMS items are portal registration confirm, webhook cutover, STOP/START from a handset, and the live smoke script — not “SMS cannot send.” Docs: BUILD-PROGRESS Telnyx checklist, Telnyx-Setup-Guide, 06-Integration-Contracts, 11-Setup-Guide §3.4/§4.6.
- [2026-09-26] — Ops — **Staging soak completed** (at least 10 real events on staging). Setup guide Part 4 gate cleared.
- [2026-09-26] — Legal — Privacy Policy and Terms updated: Telnyx as SMS provider, camera/contacts permissions, Play Data Safety table, canonical URLs `https://letssplyt.com/privacy.html` and `/terms.html`. Run `cd mobile && npm run sync:legal-docs` after further legal edits.
